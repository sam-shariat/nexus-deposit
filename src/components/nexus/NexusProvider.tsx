...
const NexusProvider = ({
  ...
  // NexusSDK uses browser-only APIs (indexedDB, etc.) so it can only be
  // constructed on the client.  Next.js aliases the import to `false` on
  // the server, which makes `NexusSDK` undefined during SSR.
  ...
  const initializeNexus = async (provider: EthereumProvider) => {
    if (!sdk) throw new Error("Nexus SDK not available (SSR?)");
    setLoading(true);
    try {
  ...
  const handleInit = async (provider: EthereumProvider) => {
    if (!sdk) throw new Error("Nexus SDK not available");
